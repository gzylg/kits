...
func CBCEncrypt(plainText, key []byte, ivDes ...byte) ([]byte, error) {
	if len(key) != 8 {
	...
	paddingText := pkcs5.Padding(plainText, block.BlockSize())
...
func CBCDecrypt(cipherText, key []byte, ivDes ...byte) ([]byte, error) {
	if len(key) != 8 {
		return nil, ErrKeyLengtheEight
	}
	block, err := des.NewCipher(key)
	...
	unPaddingText, err := pkcs5.UnPadding(plainText)
	if err != nil {
		return nil, err
	}
